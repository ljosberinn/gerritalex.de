...
Yes, specs can and should have different fantasies. Yes, <WowheadLink kind="spell" id={387103} icon="spell_fire_fire">Ruin</WowheadLink> probably shouldn't have 30% either. But if both of these specs have a similar relation to crits, then maybe their crit damage talents should be tuned similarily too.
...
Honorary mention of <WowheadLink id={129227} kind="npc">Azerokk</WowheadLink>, second boss of The MOTHERLODE!!. Commonly lusted and still commonly takes about 4 minutes to kill.
...
Remember when on Ky'veza, Mages were targeting god knows what and use `@focus` on all spells in order to abuse a bug known since Beta to not proc <WowheadLink id={444735} icon="inv_datacrystal07" kind="spell">Embedded Arcane Splinter</WowheadLink> and stopped in an instant the moment people picked up on it?
...
> Individuals (...) selling boost or assistance in (...) dungeon (...) activities for gold is allowed but can only be advertised in-game through the Trade Services chat channel. This includes any and all references to "tips", "tipping", or other synonyms of these words. (...) This means there should be no cross-realm advertising (...). 
> - [Battle.net Support Page](https://eu.support.blizzard.com/en/article/187406).
...
1. resilient keys should be the key level you've completed +1
  - this entirely removes "homework keys" which currently are still a thing, albeit less than before
2. resilient keys become only resileint if everyone in the party has the same resilient key level
  - clearly aimed at preventing boosting/selling. not opposed to this, but a bit one sided
3. resilient keys have charges instead of being permanently resilient
