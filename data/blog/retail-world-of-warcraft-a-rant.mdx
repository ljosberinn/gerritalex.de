...
It's important to remind both myself and the readers that - just like Dratnos's video - this is is first and foremost a love letter to the game. We all know however that it's not flawless. In my opinion, _most_ issues could be solved by better communication. But I'm getting ahead of myself here.
...
- as considerably competitive player I have certain expectations of endgame content
- as co-maintainer of [Jundies Plater](https://wago.io/ak3iS95aa) and a bunch of complex WeakAuras, I'm also considerably familiar with the WoW interface
...
Did you know the <WowheadSpecIcon spec="destruction-warlock">Destruction</WowheadSpecIcon> talent <WowheadLink id={457025} kind="spell" icon="inv_artifact_dimensionalrift">Dimension Ripper</WowheadLink> doesn't actually have a 5% chance, but instead 2.5%? Could of course be a tooltip bug only, but 2.5% would not only be a very underwhelming near-capstone talent, but also a surprising number in general.
...
<WowheadLink id={444088} kind="spell" icon="inv_shadowflames_wave">Consume Flame</WowheadLink> ignores damage taken modifiers.
...
- simple numeric mistakes are being made, e.g. recently <WowheadSpecIcon spec="subtlety-rogue">Subtlety Rogue</WowheadSpecIcon> was buffed by 10% in weekly class tuning but it should've been only 5%.
...
- in March, <WowheadSpecIcon spec="mistweaver-monk">Mistweaver Monk</WowheadSpecIcon> received only half of its tuning, resulting in the spec being extremely strong until the accompanying & promised 25% damage nerf was also deployed
...
I'm not here to debate whether that specific ability nor whether the AoE of <WowheadSpecIcon spec="shadow-priest">Shadow Priest</WowheadSpecIcon>s in general needs this much attention.
...
## Repeated Mistakes with Dungeon (Encounter) Design
...
- <WowheadLink id={207207} kind="npc">Voidstone Monstrosity</WowheadLink>: inverse the encounter - do damage until 70%/40%, then you need to break the shield in time while repeatly preventing the adds from spawning
...
What came from it? Absolutely nothing. It's also extra disappointing because [NetEase took the opposite approach and removed the title from 9000 players, issueing harsher penalties to some.](https://www.wowhead.com/news/netease-removes-mythic-titles-from-8-992-chinese-players-375964).
...
If you want people to stop abusing exploits, there need to be consequences. It doesn't need to be as harsh like NetEase putting up the character names of those that exploited. Hand out some 2 week bans, strip away seasonal rewards. Give people a reason to not exploit, otherwise "exploit early, exploit often" becomes more than a meme.
...
What surprised me most about this is that they should've known better and not announce it this early. Hell, it dwelled on PTR without any iteration for around 8 weeks before getting removed and only vaguely hinted on to be added at some later point in some different fashion.
...
That no jewelry beyond very rare items is available feels very arbitrary however. Why are all weapons included, even if they don't have a special effect? Yes, weapons are a larger source of power but why draw a line in general?
...
Now you've replaced chase items from raid that at least had a change to drop on a weekly basis with chase items from the Great Vault because Myth track dungeon drops aren't a thing.
...
We're currently living in a WeakAuras world where everything is possible: want the Lich King dancing while Lust is running? Sure. Want a bunny hopping across your screen for the duration of Arcane Surge? No problem.
...
## Lack of Interface Changes to Old Elements
...
Let me select multiple dungeons. Let me set thresholds for itemlevel and/or dungeon score (not that I personally care about these but just mentioning features of the addon here). Let me set a note that's automatically used when applying to groups with a single click. Sizing. Draggability.
