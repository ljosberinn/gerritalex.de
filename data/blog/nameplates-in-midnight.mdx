...
To get this out of the way - we'll not focus on visual customization here and instead solely focus on feature availability or lack thereof.

Wowhead already covered most of this on their post [What's Changing With Blizzard Nameplates? - Everything You Need to Know](https://www.wowhead.com/news/whats-changing-with-blizzard-nameplates-everything-you-need-to-know-378775?utm_source=xepheris-nameplates).
...
<center>
  <figure>
    <img src="/static/images/nameplates-midnight/stacking-nameplates.jpg" alt="part new of the Midnight nameplate options" loading="lazy" />
    <figcaption>
...
- ⚠️ Plater allows renaming NPCs by id individually, basically a nickname system
...
- ❌ Plater will no longer be able to customize unit names as all surrounding info is now secret
...
After:
- ✔️ Midnight Nameplate Options seem to mostly use the same list of auras to track as Retail
...
##### Verdict
Tough one to judge properly for me as I don't play any dot-heavy specs but I feel like making this at the very least an option should be trivial. In either case, the current ordering logic seems questionable if it doesn't include implicit ordering by duration that eluded me in the code.
...
- ✔️ addons can provide filtering via allow and deny lists
...
<center>
  <figure>
    ...
    <figcaption>yep, that's up to 14 debuffs</figcaption>
  </figure>
</center>
...
<center>
  <figure>
    ...
    <figcaption>hiding non-gameplay altering auras, down to 8 max</figcaption>
  </figure>
</center>
...
<center>
  <figure>
    ...
    <figcaption><Deathmark branch="beta" /> has been adjusted and poisons don't show anymore, down to 5 max</figcaption>
  </figure>
</center>
...
- ✔️ addons can allow highlighting special auras
...
- ⚠️ Plater likely will be able to separately display these
...
#### Conditional Unit Name Color Change
The ability to conditionally change the color of the unit name.

Before:
- ❌ Retail Default Nameplates names never change color
- ✔️ addons can change the color of the unit name

After:
- ❌ Midnight Nameplate Options never change the unit name color
- ❌ addons won't be able to change the unit name color either as aura info is secret
...
- ⚠️ Plater doesn't hide nameplates by itself
...
- ❌ Plater won't be able to hide nameplates anymore as the required info (npc id) is secret
...
<center className="flex xl:gap-2 justify-center flex-col xl:flex-row">
  <img src="/static/images/nameplates-midnight/hide-nameplate-retail-off.jpg"  className="xl:max-w-1/2" alt="" loading="lazy" />
  <img src="/static/images/nameplates-midnight/hide-nameplate-retail-on.jpg" className="xl:max-w-1/2"  alt="" loading="lazy" />
</center>

And of course, in Midnight:

<center>
    <video src="/static/images/nameplates-midnight/hide-nameplate-midnight.mp4" type="video/mp4" controls muted loop autoPlay />
</center>
...
### New CVars
And introducing a couple new ones, all related to the new options.
```
