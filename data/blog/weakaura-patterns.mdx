...
function (states, event, ...)
  if event == "COMBAT_LOG_EVENT_UNFILTERED" then -- for the sake of a high-frequency example
    -- do evil
...
[Breath of Eons](https://www.wowhead.com/spell=409632/breath-of-eons) applies a debuff with a dynamic duration on every target the casting Evoker his flying over, meaning we have a variable application time, duration and amount of affected targets.
 ...
 if aura_env.config.includeContributor and individualMax > 0 and maxSource ~= nil then
  ...
  if name then
   return string.format(
    ...
    string.format("%1.f", individualMax / total * 100)
   )
  end
 end
