...
- update <Prescience />/<ShiftingSands /> smart-targeting logic to the following:
  ...
  - self unless buffed
  - other <AugmentationEvoker plural />
...
#### Alpha Phase 4
- prevent cancelaura'ing <EbonMight /> (and honestly, any <AugmentationEvoker abbreviated /> buff)
- please, finally fix the essences. we're plagued with this for ages now
  - ![](/static/images/essence-bug.gif)

#### Alpha Phase 2
- swap <ArcaneReach /> with <BestowWeyrnstone />
- provide an alternative display for Essences
  - frankly, the default display for this is _bad_
...
#### General
- [x] <SensePower /> doesn't seem to track anything for:
  - <UnholyDeathKnight />
  - <AssassinationRogue />
- [ ] there's a long list of class abilities that do not contribute to <BreathOfEons /> as they're missing the `Allow Class Ability Procs` flag
  - [this list is sourced from simc](https://github.com/WoWAnalyzer/WoWAnalyzer/blob/the-war-within/src/analysis/retail/evoker/augmentation/modules/util/abilityFilter.ts#L329-L690), so there may be some minor errors
- [ ] <ObsidianScales /> shows the same tooltip as before regardless of whether you're talented into <RenewingBlaze /> or not
- [ ] <CommandSquadron kind="buff" /> does not work when talented out of <Maneuverability />
- [ ] <Prescience /> from <MotesOfPossibility /> doesn't seem to be able to crit when talented into <DoubleTime />
- [ ] <EbonMight /> does not update when a player dies (stays at a lower value than it should)
- [ ] <EbonMight /> does  not spread to more players if they come in range/get resurrected
  - this enables on-pull outrange meta just like in Dragonflight Season 2
- [x] <EbonMight /> does not update when a player `/cancelaura`s it
- [ ] when using <BreathOfEons /> without <EbonMight /> active, its scaling factor will depend on the last affected target count of <EbonMight />
  - e.g. if it was at 1% because you buffed lots of players before on say a world boss and then use it in solo content, it still does only 1%
  - it resets to 15% on a relog or when casting <EbonMight />
- [ ] neither your delve companion nor followers in Follower Dungeons can be buffed by <EbonMight kind="delves" /> (id: 426404)
- [ ] healer Valeera (Midnight Delve Companion) cannot receive <SourceOfMagic /> nor <SpatialParadox /> - Brann could receive them 
- [x] <Timelessness /> still only lasts for 30 minutes
...
- [x] <MasterOfDestiny /> is unchanged despite <ThreadsOfFate /> being removed
- [x] <Duplicate /> does not increase <Upheaval /> damage by 25%
- [x] neither <TimeSkip /> nor <InterwovenThreads /> affect <FireBreath />
- [x] the Essence Bar is completely broken for all evokers and throws LUA errors
  - `...s/Blizzard_UnitFrame/Mainline/EssenceFramePlayer.lua:109: calling 'Stop' on bad self (Usage: self:Stop(). Secret values are not allowed for this argument.)`
  - `...s/Blizzard_UnitFrame/Mainline/EssenceFramePlayer.lua:89: calling 'IsShown' on bad self (Usage: local isShown = self:IsShown(). Secret values are not allowed for this argument.)`

#### Cooldown Manager
- [ ] <VerdantEmbrace /> is missing from the list of Utility Cooldowns
  - it is present for both other specs
- [ ] <Expunge /> is missing from the list of Utility Cooldowns
  - [x] it is missing for <PreservationEvoker abbreviated /> as well
- [x] offers to track <ImminentDestruction /> under Buffs twice
- [ ] offers to track <OppressingRoar /> under Buffs, but doesn't work ~~with or~~ without <Overawe />
- [x] does not track <MassEruption /> - always shows as unlearned
- [x] does not track <DeepBreath /> cooldown when not talented into <BreathOfEons />
  - largely a leveling concern
- [x] when talenting out of <BreathOfEons /> for testing purposes of the above, continues to show <BreathOfEons />
- [ ] does not track <BreathOfEons /> cooldown when talenting out of <Maneuverability /> until you relog
- [x] does not track <CommandSquadron />
- [ ] attempts to track <InterwovenThreads />, which is passive
  - probably because it's a spell override to <TimeSkip />
- [x] does not track <MomentumShift />
- [ ] tracks <Duplicate /> with the wrong ids (uses spell id 1259175 instead of 1259173) 
- [x] does not track <Duplicate />
  - not the summon duration, but the aura increasing <Eruption /> & <Upheaval /> Damage
- [x] still offers to track <Unravel /> which now is a passive
- [x] still offers to track <GoldenOpportunity /> which now is a passive
- [x] offers to track <Maneuverability /> under buffs which is a passive
  - doesn't seem to actually work anyway
- [x] offers to track <RenewingBlaze /> under Utility Cooldowns
  - this also currently just doesn't work while <RenewingBlaze /> is active
- [x] does not offer to track <RenewingBlaze /> under Buffs
- [x] tracks <EssenceBurst /> even when it's not active
...
- [x] does not track <TemporalBurst />

##### Nameplates
- [x] <Bombardments /> do not show up as debuff at all
- [ ] <Landslide /> does not show up under "Shared CC"
- [ ] <OppressingRoar /> does not show up under "Shared CC"
- [x] the <Duplicate /> <FireBreath /> dot shows up on nameplates (id: 431164)


## Beta Changes December 4th

[Source](https://www.wowhead.com/news/healer-burst-healing-reduced-midnight-beta-development-notes-for-december-3rd-379505)

### Changes
> - Personal and pet damage of all spells reduced by 15%.
> - <Duplicate /> no longer increases <EbonMight />'s self damage increase by 100% while it is active.
> - <Duplicate />'s 100% increase to <EbonMight /> no longer snapshots.
> - Midnight Season 1 2-set bonus extends <EbonMight /> by 0.25 seconds (was 0.5 seconds).

### Thoughts

The bugfixes are welcome of course.

The nerf to personal damage is questionable. What is the goal with <AugmentationEvoker abbreviated /> here? <Duplicate /> in general adds more personal damage, which has been the trend for a while now, implicitly hinting that the spec has a design goal of less buff damage.

And now personal damage gets nerfed again? Getting mixed signals here. Unfortunately, folks tasked making changes on Evoker specs only extremely rarely communicate, hardly even in Patch Notes, so we're only left to guess.

## Alpha Phase 4
October 22, 2025, [Source](https://www.wowhead.com/news/phase-four-midnight-alpha-development-notes-voidstorm-tier-and-more-378986)

### Changes

> - Chronowarden: New Talent: <NozdormuAdept /> - <Prescience /> cooldown reduced by 2 seconds and it grants 1% additional critical strike chance.
> - Chronowarden: <ThreadsOfFate /> has been removed.
> - Class: <Quell /> moved to the <AugmentationEvoker /> talent tree
> - <ImminentDestruction branch="beta" /> has been redesigned - <BreathOfEons /> reduces the Essence cost of your next 6 <Eruption plural /> by 1.
> - <MoltenEmbers /> has been redesigned - Enemies affected by <FireBreath />'s damage over time effect take 25% increased damage from your Black spells.

### Thoughts
- the removal of <ThreadsOfFate /> doesn't come as a surprise after the introduction of <Chronoboon />
  - unfortunately, <MasterOfDestiny /> was forgotten and still interacts with the now-removed <ThreadsOfFate />
- <Quell /> (and related talents) getting moved around doesn't affect <AugmentationEvoker abbreviated /> in particular but is part of a [design shift re interrupts for healers](https://us.forums.blizzard.com/en/wow/t/updates-to-healer-specializations-in-midnight/2189090) which I'm just surfacing here in case you've missed it
- the <ImminentDestruction branch="beta" /> redesign I'm cautiously positive about:
  - notably, the buff for it is now 30 seconds long, not 15 as before
  - logs suggest I'm currently casting an average of 9 <Eruption />s per <ImminentDestruction />
...
- <MoltenEmbers /> in its initial form was already a buff to Black spells during <FireBreath /> and now we're back to that, except its 25% instead of 20%
  - fine by me, encourages casting rank 1 <FireBreath /> however which is at odds with <LeapingFlames />
  - since <InfernosBlessing /> got tuned, these at least _seem_ to be good options now, once again  

## Alpha Phase 3

October 15th 2025, [Source](https://www.wowhead.com/news/pandemic-dot-assistance-on-cooldown-manager-development-notes-for-midnight-phase-378897)

### Changes

No tuning/talent changes for Evoker in this phase, however:
- talent changes from the previous week are now _actually_ in and testable
- we're getting an early look at the next tier sets for <AugmentationEvoker />
...
> Evoker Augmentation 12.0 Class Set 2pc: <Eruption /> damage increased by 15% and it extends <EbonMight /> by an additional 0.5 sec.

Yet another <Eruption /> modifier which I don't mind. Extending <EbonMight /> longer is certainly interesting.

> Evoker Augmentation 12.0 Class Set 4pc: While <EbonMight /> is active, empower spells deal 20% more damage and cool down 20% faster.

Since you should only use empower spells during <EbonMight />, this is putting more emphasis on good play. Making them more available during <EbonMight /> also leads to more <EbonMight /> uptime itself in addition to <ShiftingSands />.

Neither bonuses something you actively have to think about at all but honestly, pretty decent in general design wise.

#### Talent Positioning
- <ProtractedTalons /> gating <RegenerativeMagic /> does not feel good:
  - <AzureStrike /> is a button you currently cast maybe once every two minutes
  - <ProtractedTalons /> doesn't interact with anything of relevance for <AugmentationEvoker abbreviated />, but in all fairness, <RegenerativeMagic /> isn't very strong for <AugmentationEvoker abbreviated /> either
- pretty happy with all the other shuffling that happened
